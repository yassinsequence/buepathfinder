...
async function testCVAnalysis() {
  try {
    console.log('Testing CV Analysis API with gemini-2.0-flash...');
    const model = genAI.getGenerativeModel({
      model: 'gemini-2.0-flash',
    });
...
3. **Recommended Jobs in Egypt**: Suggest 5 job opportunities
4. **Skill Gaps**: What skills are missing
5. **Learning Recommendations**: Specific courses
...
(async () => {
  ...
  const cvSuccess = await testCVAnalysis();

  console.log('\n=== TEST SUMMARY ===');
  console.log(`Chat API: ${chatSuccess ? '✓ WORKING' : '✗ FAILED'}`);
  console.log(`CV Analysis API: ${cvSuccess ? '✓ WORKING' : '✗ FAILED'}`);
})();
